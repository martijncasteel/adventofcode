...
def game(p1, p2, game_number = 1):
  ...
  while True:
    ...
    print(f'Player 1\'s deck: { ", ".join([str(c) for c in p1.cards]) }')
    print(f'Player 2\'s deck: { ", ".join([str(c) for c in p2.cards]) }')

    # if this game occurred before player 1 wins
    if (p1.calculate(), p2.calculate()) in history:
      print('-- history repeats itself, player 1 wins --')
      return p1

    history.add((p1.calculate(), p2.calculate()))

    # draw a card each, current play
    draw = (round, p1.draw(), p2.draw())
    print(f'Player 1 plays: { draw[1] }')
    print(f'Player 2 plays: { draw[2] }')

    if len(p1.cards) >= draw[1] and len(p2.cards) >= draw[2]:
      # play another game as rules depict
      print('Playing a sub-game to determine the winner...')

      winner = game(p1.copy(draw[1]), p2.copy(draw[2]), game_number + 1)
    ...
    if winner == p1:
      ...
      print(f'Player 1 wins round {round} of game {game_number}!')
      ...
      print(f'Player 2 wins round {round} of game {game_number}!')
