...
class Cube:
  ...
  def compute(self, previous):
    ...
    for w in range(self.vector[3] -1, self.vector[3] + 2):
      for z in range(self.vector[2] -1, self.vector[2] + 2):
        for y in range(self.vector[1] -1, self.vector[1] +2):
          for x in range(self.vector[0] -1, self.vector[0] +2):
            ...
            if (x,y,z,w) == self.vector:
              continue

            if (x,y,z,w) not in previous:
              continue
            
            if previous[(x,y,z,w)].active:
              sum += 1
...
def print_pockets(dict):
  ...
  for cube in sorted(dict.values()):
    ...
    if not previous:
      print(f'z={cube.vector[2]}, w={cube.vector[3]}')
      print(cube, end ='')

    elif previous.vector[2] != cube.vector[2] or previous.vector[3] != cube.vector[3]:
      print(f'\n\nz={cube.vector[2]}, w={cube.vector[3]}\n')
      print(cube, end ='')
...
with open('input', 'r') as file:
  for y, line in enumerate(file):
    ...
    for x, char in enumerate(characters):
      pockets[(x,y,0, 0)] = Cube((x, y, 0, 0), char == "#")
